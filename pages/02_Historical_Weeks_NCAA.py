...
def ncaaf_ats(row):
    ...
    picked_name = " ".join(pick.split()[:-1]).strip()   # everything except the spread
    pick_team = picked_name == team
    pick_opp  = picked_name == opp

    if pick_team:
        # Spread already belongs to Team
        pick_spread = spread
        ats_margin = team_score + pick_spread - opp_score

    elif pick_opp:
        # Opponent gets the opposite spread
        pick_spread = -spread
        ats_margin = opp_score + pick_spread - team_score
