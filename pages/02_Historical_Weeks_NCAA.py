...
def ncaaf_ats(row):
    ...
    pick_team = pick.startswith(team.lower())
    pick_opp = pick.startswith(opp.lower())


    # Spread applied to the picked side
    if pick_team:
        pick_spread = spread
    elif pick_opp:
        pick_spread = -spread
    else:
        pick_spread = 0  # fallback, shouldn't happen

    ats_margin = margin + pick_spread
