...
# -----------------------------------------
# Compute ATS metrics (pick-based)
# -----------------------------------------
season_df["actual_margin"] = season_df["TeamScore"] - season_df["OppScore"]

def compute_pick_spread(row):
    pick = row["recommended_pick"]
    team = row["Team"]
    spread = row["spread_value"]
    pick_team = pick.startswith(team)
    return spread if pick_team else -spread

season_df["pick_spread"] = season_df.apply(compute_pick_spread, axis=1)
...
season_df["ATS_Result"] = season_df.apply(ncaaf_ats_result, axis=1)
