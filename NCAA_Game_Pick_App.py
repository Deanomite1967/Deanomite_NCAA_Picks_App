...
if not os.path.exists(TEAM_MASTER_PATH):
    ...
    print("TEAM MASTER CREATED:", TEAM_MASTER_PATH)
else:
    print("TEAM MASTER ALREADY EXISTS:", TEAM_MASTER_PATH)


def load_week_spreads_ncaaf(week_number):
    path = f"mount/src/deanomite_ncaa_picks_app/data/Week{week_number}_Spreads.csv"
    df = pd.read_csv(path)
...
def load_week_results_ncaaf(week_number):
    ...
    path = f"mount/src/deanomite_ncaa_picks_app/data/Week{week_number}_Results.csv"
...
for wk in range(0, 19):  # NCAA weeks
    path = f"mount/src/deanomite_ncaa_picks_app/data/Week{wk}_Spreads.csv"
...
for wk in range(0, 19):
    if os.path.exists(f"mount/src/deanomite_ncaa_picks_app/data/Week{wk}_Spreads.csv"):
        current_week = wk
    if os.path.exists(f"mount/src/deanomite_ncaa_picks_app/data/Week{wk}_Results.csv"):
        completed_week = wk
...
if run_button:
    ...
    if not os.path.exists("mount/src/deanomite_ncaa_picks_app/data/team_master.csv"):
        print("CREATING TEAM MASTER NOW...")
        off = pd.read_csv("mount/src/deanomite_ncaa_picks_app/data/2025_Team_Offense.csv")
        defn = pd.read_csv("mount/src/deanomite_ncaa_picks_app/data/2025_Team_Defense.csv")
        met = pd.read_csv("mount/src/deanomite_ncaa_picks_app/data/2025_Team_Metrics.csv")
        ...
        team.to_csv("mount/src/deanomite_ncaa_picks_app/data/team_master.csv", index=False)
        print("TEAM MASTER CREATED.")


    # --- Load team-level NCAA data (your 3 CSVs already merged) ---
    team_master_ncaaf = pd.read_csv("mount/src/deanomite_ncaa_picks_app/data/team_master.csv")
    ...
    if week_number > -1:
        ...
        if training_df is not None:
            save_path = f"mount/src/deanomite_ncaa_picks_app/data/Week{prev_week}_Training.xlsx"
