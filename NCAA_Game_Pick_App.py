...
def train_multiweek_model_ncaaf(up_to_week):
    ...
    # ============================================
    # TEAM NAME NORMALIZATION (PASTE THIS HERE)
    # ============================================
    TEAM_NORMALIZE = {
        "texas christian": "tcu",
    ...
    # Apply normalization
    season_df["Team"] = (
        season_df["Team"].str.strip().str.lower().map(TEAM_NORMALIZE)
        .fillna(season_df["Team"].str.strip().str.lower())
    )

    season_df["Opp"] = (
        season_df["Opp"].str.strip().str.lower().map(TEAM_NORMALIZE)
        .fillna(season_df["Opp"].str.strip().str.lower())
    )

    team_master_ncaaf["School"] = (
        team_master_ncaaf["School"].str.strip().str.lower().map(TEAM_NORMALIZE)
        .fillna(team_master_ncaaf["School"].str.strip().str.lower())
    )
    ...
    season_df = season_df.merge(
        team_master_ncaaf.add_suffix("_team"),
        left_on="Team",
    ...
    season_df = season_df.merge(
        team_master_ncaaf.add_suffix("_opp"),
        left_on="Opp",
        right_on="School_opp",
        how="left"
    )
    ...
    # Build features using suffixed columns
