...
def train_multiweek_model_ncaaf(up_to_week):
    ...
    # CLEAN MERGE — REMOVE BAD COLUMNS
    bad_cols = [
        "TSRS", "OSRS", "DSRS", "PYPA", "RYPA",
        "Pts_off", "Pts_def",
        "TSRS_opp", "OSRS_opp", "DSRS_opp",
        "PYPA_opp", "RYPA_opp",
        "Pts_off_opp", "Pts_def_opp"
    ]

    season_df = season_df.drop(columns=bad_cols, errors="ignore")
    # ---------------------------------------------------------
    # FIX: CLEAN UP MERGE COLUMNS USING THE CORRECT VARIABLE
    # ---------------------------------------------------------
    rename_map = {
        "TSRS_x": "TSRS",
        "OSRS_x": "OSRS",
    ...
    season_df = season_df.drop(columns=[c for c in season_df.columns if c.endswith("_y")], errors="ignore")
...
def get_week_picks_singleweek_ncaaf(week_number):
    ...
    st.write("=== DEBUG: RAW MERGED STATS ===")
    st.write(games.head())

    # ---------------------------------------------------------
    # FIX: CLEAN UP MERGE COLUMNS
    # ---------------------------------------------------------
    bad_cols = [
        "TSRS", "OSRS", "DSRS", "PYPA", "RYPA",
        "Pts_off", "Pts_def",
    ...
    games = games.drop(columns=bad_cols, errors="ignore")
    ...
    games = games.drop(columns=[c for c in games.columns if c.endswith("_y")], errors="ignore")
...
def get_week_picks_multiweek_ncaaf(week_number):
    ...
    spreads = load_week_spreads_ncaaf(week_number)
    games   = merge_matchups_ncaaf(spreads, team_master_ncaaf)
    st.write("=== DEBUG: RAW MERGED COLUMNS ===")
    st.write(games.columns.tolist())

    # ---------------------------------------------------------
    # MERGE CLEANUP (MUST BE HERE TOO)
    # ---------------------------------------------------------
    bad_cols = [
        "TSRS", "OSRS", "DSRS", "PYPA", "RYPA",
        "Pts_off", "Pts_def",
