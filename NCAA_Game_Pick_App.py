...
def create_features_ncaaf(df):
    ...
    # Differences
    df["adj_off_diff"] = df["adj_off"] - df["adj_off_opp"]
    df["adj_def_diff"] = df["adj_def"] - df["adj_def_opp"]

    # Normalize SOS diff (still useful)
    df["sos_diff_norm"] = df["sos_diff"] / 10.0
...
def predict_games_ncaaf(feats):
    ...
    # NEW prediction method (direct replacement)
    feats["model_pred"] = (
        (feats["Pts_off"] - feats["Pts_def_opp"]) +
        (feats["Pts_def"] - feats["Pts_off_opp"])
    ) + feats["sos_diff_norm"] + (feats["tsrs_diff"]/2)

    feats["model_pred"] = feats["model_pred"] * -1
