...
def train_multiweek_model_ncaaf(up_to_week):
    season_df = season_df[["Team", "Opp", "spread_value_x", "actual_margin"]].copy()
    ...
    # TEAM merge (IDENTICAL to matchup merge)
    ...
    # OPP merge (IDENTICAL to matchup merge)
    season_df = season_df.merge(
    ...
    # --- CLEAN OPP COLUMNS: drop _opp_x, rename _opp_y to canonical ---
    opp_rename = {}

    # SRS / efficiency
    for base in ["OSRS", "DSRS", "TSRS", "PYPA", "RYPA"]:
        x_col = f"{base}_opp_x"
        y_col = f"{base}_opp_y"
        if x_col in season_df.columns:
            season_df = season_df.drop(columns=[x_col])
        if y_col in season_df.columns:
            opp_rename[y_col] = f"{base}_opp"

    # Points
    for base in ["Pts_off", "Pts_def"]:
        x_col = f"{base}_opp_x"
        y_col = f"{base}_opp_y"
        if x_col in season_df.columns:
            season_df = season_df.drop(columns=[x_col])
    ...
    if opp_rename:
        season_df = season_df.rename(columns=opp_rename)


    # After TEAM + OPP merges
    st.write("SEASON_DF COLUMNS:", season_df.columns.tolist())
...
def get_week_picks_multiweek_ncaaf(week_number):
    ...
    spreads = load_week_spreads_ncaaf(week_number)
    games   = merge_matchups_ncaaf(spreads, team_master_ncaaf)
    sample = games[(games["Team"] == "oklahoma") & (games["Opp"] == "utep")]
    st.write("OKLAHOMA vs UTEP TSRS_team:", sample["TSRS_team"])
    st.write("OKLAHOMA vs UTEP TSRS_opp:", sample["TSRS_opp"])
    #st.write("TEAM MASTER COLUMNS:", team_master_ncaaf.columns.tolist())
    #st.dataframe(team_master_ncaaf.head())
