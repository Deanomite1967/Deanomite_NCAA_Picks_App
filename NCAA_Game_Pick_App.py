...
def get_week_picks_singleweek_ncaaf(week_number):
    ...
    # ---------------------------------------------------------
    # MANUAL MODEL THAT MATCHES YOUR RF OUTPUT RANGE
    # ---------------------------------------------------------
    # This compresses your weighted diffs into sane values.
    # ---------------------------------------------------------
    feats["model_pred"] = (
          feats["tsrs_diff"] * 0.05
        + feats["osrs_diff"] * 0.04
        + feats["dsrs_diff"] * 0.04
        + feats["pypa_diff"] * 0.02
        + feats["rypa_diff"] * 0.02
        + feats["pts_off_diff"] * 0.03
        - feats["pts_def_diff"] * 0.03
    )

    # Blend with Vegas (your original NFL-style blend)
