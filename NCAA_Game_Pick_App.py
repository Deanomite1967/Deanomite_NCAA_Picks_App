...
team.to_csv("c:/DraftKings/NCAA/team_master.csv", index=False)

# --- Load team-level NCAA data (your 3 CSVs already merged) ---
team_master_ncaaf = pd.read_csv("C:/DraftKings/NCAA/team_master.csv")
...
def load_week_spreads_ncaaf(week_number):
    path = f"C:/DraftKings/NCAA/Week{week_number}_Spreads.csv"
    df = pd.read_csv(path)
...
def load_week_results_ncaaf(week_number):
    ...
    path = f"C:/DraftKings/NCAA/Week{week_number}_Results.csv"
...
for wk in range(1, 16):  # NCAA weeks
    path = f"C:/DraftKings/NCAA/Week{wk}_Spreads.csv"
...
if run_button:
    ...
    if training_df is not None:
        training_df.to_excel(f"C:/DraftKings/NCAA/Week{week_number}_Training.xlsx", index=False)
