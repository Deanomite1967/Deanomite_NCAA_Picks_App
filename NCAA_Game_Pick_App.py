...
def create_features_ncaaf(df):
    ...
    # Normalize SOS_diff to a reasonable scale
    df["sos_diff_norm"] = df["sos_diff"] / 20.0

    # ---------------------------------------------------------------
    # SOS‑ADJUSTED OFFENSIVE & DEFENSIVE SCORING
    # ---------------------------------------------------------------
    df["adj_off"]     = df["Pts_off"]     * (1 + df["SOS"]     / 20.0)
    df["adj_def"]     = df["Pts_def"]     * (1 + df["SOS"]     / 20.0)
    df["adj_off_opp"] = df["Pts_off_opp"] * (1 + df["SOS_opp"] / 20.0)
    df["adj_def_opp"] = df["Pts_def_opp"] * (1 + df["SOS_opp"] / 20.0)
...
def predict_games_ncaaf(feats):
    ...
    feats["model_pred"] = (
        ...
        + (feats["adj_off_diff"] * 0.80)
        - (feats["adj_def_diff"] * 0.80)
