...
def predict_games_ncaaf(feats):
    ...
    df["term_tsrs"]   = df["tsrs_diff"]    * 0.60
    df["term_osrs"]   = df["osrs_diff"]    * 0.25
    df["term_dsrs"]   = df["dsrs_diff"]    * 0.25
    df["term_pypa"]   = df["pypa_diff"]    * 0.30
    df["term_rypa"]   = df["rypa_diff"]    * 0.30
    df["term_off"]    = df["adj_off_diff"] * 0.85
    df["term_def"]    = df["adj_def_diff"] * 0.85
    df["term_sos"]    = df["sos_diff_norm"] * 0.60
...
def build_training_data_ncaaf(week_number):
    ...
    st.dataframe(
        df[[
            "Team", "Opp",
            "spread_value",
            "term_tsrs", "term_osrs", "term_dsrs",
            "term_pypa", "term_rypa",
            "term_off", "term_def", "term_sos",
            "model_pred"
        ]]
    )
