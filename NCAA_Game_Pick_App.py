...
def train_multiweek_model_ncaaf(up_to_week):
    ...
    st.write("=== RAW SEASON_DF BEFORE ANY MERGES ===")
    st.write(season_df.columns.tolist())
    st.dataframe(season_df.head(50))

    # Merge team stats
    season_df = merge_matchups_ncaaf(season_df, team_master_ncaaf)
    # --- RESTORE ORIGINAL COLUMN NAMES AFTER MERGE ---
    rename_map = {
        "TSRS_x": "TSRS",
        "OSRS_x": "OSRS",
        "DSRS_x": "DSRS",
    ...
    drop_cols = [col for col in season_df.columns if col.endswith("_y")]
    season_df = season_df.drop(columns=drop_cols)
...
def get_week_picks_singleweek_ncaaf(week_number):
    ...
    feats["model_pred"] = (
        ...
        + feats["pypa_diff"] * 0.2
        + feats["rypa_diff"] * 0.2
        + feats["pts_off_diff"] * 0.3
        - feats["pts_def_diff"] * 0.3
    )

    feats["model_pred"] = (0.6 * feats["model_pred"]) + (0.4 * feats["spread_value"])
