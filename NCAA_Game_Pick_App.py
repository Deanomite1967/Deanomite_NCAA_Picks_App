...
def predict_games_ncaaf(feats):
    ...
    feats["model_pred"] = (
        (feats["tsrs_diff"]    * 0.60)
        + (feats["osrs_diff"]  * 0.25)
        + (feats["dsrs_diff"]  * 0.25)
        + (feats["pypa_diff"]  * 0.30)
        + (feats["rypa_diff"]  * 0.30)
        + (feats["adj_off_diff"] * 0.85)
        - (feats["adj_def_diff"] * 0.85)
        + (feats["sos_diff_norm"] * 0.60)
    )
...
def build_training_data_ncaaf(week_number):
    ...
    st.write("Columns after merging spreads + results:", df.columns.tolist())
    ...
    st.write("Columns after merging team metrics:", df.columns.tolist())
    ...
    drop_cols = [
        "OSRS_y", "DSRS_y", "TSRS_y", "PYPA_y", "RYPA_y",
        "Pts_off_y", "Pts_def_y", "SOS_y",
        "OSRS_opp_y", "DSRS_opp_y", "TSRS_opp_y",
        "PYPA_opp_y", "RYPA_opp_y",
        "Pts_off_opp_y", "Pts_def_opp_y", "SOS_opp_y"
    ]
    ...
    st.write("Columns BEFORE dropna:", df.columns.tolist())
