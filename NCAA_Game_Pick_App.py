...
def predict_games_ncaaf(feats):
    ...
    # Ensure spread_value exists for both single-week and training pipelines
...
def build_training_data_ncaaf(week_number):
    ...
    # Ensure spread_value exists in spreads
    if "spread_value" not in spreads.columns and "Spread" in spreads.columns:
        spreads["spread_value"] = spreads["Spread"].astype(float)
    ...
    # Rename suffixed metric columns
    rename_map = {
        "TSRS_x": "TSRS",
        "TSRS_opp_x": "TSRS_opp",
        "OSRS_x": "OSRS",
    ...
    # Drop rows for teams not playing this week
    required_cols = [
        "TSRS", "TSRS_opp", "OSRS", "OSRS_opp", "DSRS", "DSRS_opp",
        "PYPA", "PYPA_opp", "RYPA", "RYPA_opp",
        "Pts_off", "Pts_off_opp", "Pts_def", "Pts_def_opp",
        "SOS", "SOS_opp",
    ]
    ...
    # *** CRITICAL FIX: rename spread_value AFTER feature creation ***
    if "spread_value_x" in df.columns:
        df = df.rename(columns={"spread_value_x": "spread_value"})
    if "spread_value_y" in df.columns:
        df = df.drop(columns=["spread_value_y"])
...
def build_training_data_ncaaf(week_number):
    st.write(f"🔍 DEBUG: Building training data for week {week_number}")

    if week_number < 0:
        st.write("Week number < 0 — returning None")
        return None

    spreads = load_week_spreads_ncaaf(week_number)
    results = load_week_results_ncaaf(week_number)

    st.write("📄 DEBUG: Spreads columns:", list(spreads.columns))
    st.write("📄 DEBUG: Results columns:", list(results.columns) if results is not None else "None")

    if results is None:
        st.write("❌ No results file found — returning None")
        return None

    # Merge spreads + results
    df = spreads.merge(results, on=["Team", "Opp"], how="inner")
    st.write("📄 DEBUG: After merging spreads + results:", list(df.columns))

    # Normalize names BEFORE merging with team master
    df["Team"] = df["Team"].str.strip().str.lower()
    df["Opp"]  = df["Opp"].str.strip().str.lower()

    st.write("🔍 DEBUG: Unique Teams in training:", df["Team"].unique()[:20])
    st.write("🔍 DEBUG: Unique Opps in training:", df["Opp"].unique()[:20])

    # Merge team metrics (including SOS)
    df = merge_matchups_ncaaf(df, team_master_ncaaf)
    st.write("📄 DEBUG: After merging team master:", list(df.columns))
    ...
    st.write(f"🔍 DEBUG: Dropped {before - after} rows for teams not playing this week")

    # Create features
    st.write("🔧 DEBUG: Creating features...")
    df = create_features_ncaaf(df)

    st.write("📄 DEBUG: After feature creation:", list(df.columns))

    # Add model predictions
    st.write("🔧 DEBUG: Running model prediction...")
    df = predict_games_ncaaf(df)

    st.write("📄 DEBUG: After model prediction:", list(df.columns))
    st.write("🔍 DEBUG: Sample model_pred values:", df["model_pred"].head())
