...
def train_multiweek_model_ncaaf(up_to_week):
    ...
    # Build TEAM / OPP versions
    team_master_team = team_master_ncaaf.rename(
        columns={col: f"{col}_team" for col in team_master_ncaaf.columns}
    )
    team_master_opp = team_master_ncaaf.rename(
        columns={col: f"{col}_opp" for col in team_master_ncaaf.columns}
    )
    ...
    # --- HARD CLEANUP: drop duplicate / junk columns explicitly ---
    # keep only one TSRS_team and one TSRS_opp
    cols_to_drop = [
        "TSRS_team_x",  # keep TSRS_team_y
        "TSRS_opp_x",   # keep TSRS_opp_y
        "TSRS",         # base TSRS if it leaked in
    ]
    season_df = season_df.drop(columns=[c for c in cols_to_drop if c in season_df.columns], errors="ignore")

    # rename surviving TSRS columns to canonical names
    if "TSRS_team_y" in season_df.columns:
        season_df = season_df.rename(columns={"TSRS_team_y": "TSRS_team"})
    if "TSRS_opp_y" in season_df.columns:
        season_df = season_df.rename(columns={"TSRS_opp_y": "TSRS_opp"})

    # same pattern for OSRS / DSRS / PYPA / RYPA if they have _x/_y
    rename_map = {}
    for base in ["OSRS", "DSRS", "PYPA", "RYPA"]:
        team_x = f"{base}_team_x"
        team_y = f"{base}_team_y"
        opp_x  = f"{base}_opp_x"
        opp_y  = f"{base}_opp_y"

        if team_x in season_df.columns and team_y in season_df.columns:
            season_df = season_df.drop(columns=[team_x])
            rename_map[team_y] = f"{base}_team"
        elif team_y in season_df.columns:
            rename_map[team_y] = f"{base}_team"

        if opp_x in season_df.columns and opp_y in season_df.columns:
            season_df = season_df.drop(columns=[opp_x])
            rename_map[opp_y] = f"{base}_opp"
        elif opp_y in season_df.columns:
            rename_map[opp_y] = f"{base}_opp"

    if rename_map:
        season_df = season_df.rename(columns=rename_map)

    season_df["Pts_off_team"] = season_df["Pts_off"]
    season_df["Pts_def_team"] = season_df["Pts_def"]
    season_df["Pts_off_opp"]  = season_df["Pts_off_opp"]
    season_df["Pts_def_opp"]  = season_df["Pts_def_opp"]
