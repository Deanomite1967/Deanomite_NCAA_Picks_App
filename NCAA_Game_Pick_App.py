...
def build_training_data_ncaaf(week_number):
    ...
    df["Team"] = df["Team"].map(NAME_MAP).fillna(df["Team"])
    df["Opp"]  = df["Opp"].map(NAME_MAP).fillna(df["Opp"])
...
def train_multiweek_model_ncaaf(up_to_week):
    ...
    season_df["Team"] = season_df["Team"].map(NAME_MAP).fillna(season_df["Team"])
    season_df["Opp"]  = season_df["Opp"].map(NAME_MAP).fillna(season_df["Opp"])
    # --- END BLOCK ---

    # --- build explicit TEAM and OPP versions of team_master ---
    team_master_team = team_master_ncaaf.rename(
        columns={col: f"{col}_team" for col in team_master_ncaaf.columns}
    )
    team_master_opp = team_master_ncaaf.rename(
    ...
    # --- FIX: rename correct opponent TSRS column ---
    season_df = season_df.rename(columns={"TSRS_opp_y": "TSRS_opp"})
    season_df = season_df.drop(columns=["TSRS", "TSRS_opp_x"], errors="ignore")
