...
def build_training_data_ncaaf(week_number):
    ...
    # Create features (including sos_diff)
...
def build_training_data_ncaaf(week_number):
    ...
    # ---------------------------------------------------------
    # FIX: Rename suffixed columns to match prediction pipeline
    # ---------------------------------------------------------
    rename_map = {
        "TSRS_x": "TSRS",
        "TSRS_opp_x": "TSRS_opp",
    ...
    # Check for missing opponent metrics
    missing_cols = [c for c in ["TSRS", "TSRS_opp", "SOS", "SOS_opp"] if c not in df.columns]
    if missing_cols:
        st.write("❌ DEBUG: Missing columns after rename:", missing_cols)

    # Show rows where opponent metrics failed to merge
    bad_merge = df[df.isna().any(axis=1)]
    if len(bad_merge) > 0:
        st.write("❌ DEBUG: Rows with missing data after merge:")
        cols_to_show = [c for c in ["Team", "Opp", "TSRS", "TSRS_opp", "SOS", "SOS_opp"] if c in df.columns]
        st.write(bad_merge[cols_to_show])
