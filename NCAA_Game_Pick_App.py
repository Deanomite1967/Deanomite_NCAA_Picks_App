...
def predict_games_ncaaf(feats):
    feats["model_pred"] = (
          (feats["tsrs_diff"]    * 0.35)
        + (feats["osrs_diff"]    * 0.20)
        + (feats["dsrs_diff"]    * 0.20)
        + (feats["pypa_diff"]    * 0.50)
        + (feats["rypa_diff"]    * 0.30)
        + (feats["pts_off_diff"] * 0.25)
        - (feats["pts_def_diff"] * 0.25)
    )
...
def get_week_picks_singleweek_ncaaf(week_number):
    ...
    # ⭐ DEBUG: SHOW TEAM MASTER SCHOOLS
    st.write("TEAM MASTER SCHOOLS:", team_master_ncaaf["School"].unique())

    # ⭐ DEBUG: SHOW SPREADS TEAMS
    st.write("SPREADS TEAMS:", spreads["Team"].unique())

    # ⭐ DEBUG: SHOW MERGED GAME STATS
    st.write("DEBUG MERGED GAMES:", games[[
        "Team","Opp",
        "TSRS","OSRS","DSRS",
        "PYPA","RYPA",
        "Pts_off","Pts_def"
